...
const postUser = (req, res) => {
  ...
  database
    .query("INSERT INTO users(firstname, lastname, email, city, language, hashedPassword) VALUE (?, ?, ?, ?, ?, ?)", [firstname, lastname, email, city, language, hashedPassword])
    .then(([result]) => {
      res.location(`/api/${result.insertId}`).sendStatus(201);
    })
    .catch((err) => {
      console.error(err);
